  ...
  if (request.type === 'getExplanation') {
    ...
    chrome.storage.sync.get('apiKey', ({ apiKey }) => {
      ...
      fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          model: 'gpt-5-nano',
          input: `我不是很理解该段，请用中文更加通俗易懂地来详细解释: ${request.selection}。
并且请基于上下文来解释
言简意赅，100字以内。
...
以下是上下文：${request.context}`,
      ...
      .then(data => {
        if (data.output && data.output.length > 1 && data.output[1].content && data.output[1].content[0].text) {
          sendResponse({ explanation: data.output[1].content[0].text.trim() });
        } else {
